...
float dist(vec3 p, float t) {
  if (t < 0.75) return sdSphere(p, 0.5);
	else {
		float d = length(p.xy);
    float tm = time * 0.1;
    return max(max(abs(p.z), max(0., abs(d-0.8)-0.2)),
           		 fnoise(vec3(atan(p.y, p.z) + tm, d, tm * .1)) * 10.) * .1;
  }
}
...
void stepPos(in float i, in vec4 prevPos, in vec4 pos, out vec4 nextPos) {
  ...
  vel += flowPower * flow(p - time * .1 + t * 1000., .01) * .2;
